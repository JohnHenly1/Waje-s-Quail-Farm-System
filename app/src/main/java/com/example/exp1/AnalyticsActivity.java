...
public class AnalyticsActivity extends AppCompatActivity {
    ...
    // Firestore — feed inventory value (linked from FeedInventoryActivity)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        inventoryValueTv   = findViewById(R.id.feedCostValue);   // same view id, label updated in layout
        ...
        eggCollectionsListener = new ValueEventListener() {
            @Override public void onDataChange(DataSnapshot snapshot) {
                ...
                for (DataSnapshot child : snapshot.getChildren()) {
                    ...
                    if (dateKey != null && total != null) { allData.put(dateKey, total.intValue()); totalEggs += total.intValue(); }
                }
                runOnUiThread(() -> updateDashboard());
            }
            @Override public void onCancelled(DatabaseError error) { allData = new TreeMap<>(); totalEggs = 0; runOnUiThread(() -> updateDashboard()); }
        };
        eggCollectionsRef.addValueEventListener(eggCollectionsListener);
    }

    // ── Firestore feed inventory listener (links to FeedInventoryActivity) ────
    ...
    private void startLiveInternetSensor() {
        ...
        networkCallback = new ConnectivityManager.NetworkCallback() {
            @Override public void onAvailable(Network network) { runOnUiThread(() -> { View ll = findViewById(R.id.loadingLayout); if (ll != null && ll.getVisibility() == View.VISIBLE) { View nn = findViewById(R.id.noInternetSection); if (nn != null && nn.getVisibility() == View.VISIBLE) { ll.setVisibility(View.GONE); recreate(); } } }); }
            @Override public void onLost(Network network) { runOnUiThread(() -> NavigationHelper.INSTANCE.showNoInternetOverlay(AnalyticsActivity.this)); }
        };
    ...
    @Override protected void onStop() {
        super.onStop();
        if (networkCallback != null) ((ConnectivityManager) getSystemService(Context.CONNECTIVITY_SERVICE)).unregisterNetworkCallback(networkCallback);
        if (eggCollectionsListener != null) eggCollectionsRef.removeEventListener(eggCollectionsListener);
    ...
    private void startTimeUpdate() {
        ...
        Runnable updateTime = new Runnable() {
            @Override public void run() { Calendar cal = Calendar.getInstance(); if (serverTimeLabel != null) serverTimeLabel.setText(timeFormat.format(cal.getTime())); handler.postDelayed(this, 60000); }
        };
        handler.post(updateTime);
    }

    // ── Dashboard ─────────────────────────────────────────────────────────────
    // Revenue = totalEggs * ₱5.00 | Inventory Value = live Firestore sum | Net Profit = Revenue - Inventory Value
    private void updateDashboard() {
        ...
        if (profitValue      != null) {
            profitValue.setText(String.format(Locale.getDefault(), "₱%.0f", profit));
        ...
        for (Map.Entry<String, Integer> entry : allData.entrySet()) {
            try {
                Date date = sdf.parse(entry.getKey()); Calendar ec = Calendar.getInstance(); ec.setTime(date);
                int yr = ec.get(Calendar.YEAR), mo = ec.get(Calendar.MONTH), wk = ec.get(Calendar.WEEK_OF_MONTH);
                if (yr == selectedWeeklyYear && mo == selectedWeeklyMonth) weekTotals.put(wk, weekTotals.getOrDefault(wk, 0) + entry.getValue());
            } catch (Exception ignored) {}
        }
        List<Entry> lineEntries = new ArrayList<>(); List<String> weeklyLabels = new ArrayList<>();
        for (int w = 1; w <= 4; w++) { lineEntries.add(new Entry(w - 1, weekTotals.getOrDefault(w, 0))); weeklyLabels.add("Week " + w); }
        LineDataSet lineSet = new LineDataSet(lineEntries, "Egg Count");
        lineSet.setColor(Color.parseColor("#466d1d")); lineSet.setCircleColor(Color.parseColor("#466d1d")); lineSet.setLineWidth(3f); lineSet.setDrawValues(true);
        weeklyChart.setData(new LineData(lineSet)); weeklyChart.getXAxis().setValueFormatter(new IndexAxisValueFormatter(weeklyLabels));
        weeklyChart.getXAxis().setLabelRotationAngle(-45f); weeklyChart.getXAxis().setGranularity(1f); weeklyChart.getXAxis().setLabelCount(weeklyLabels.size());
        weeklyChart.animateX(800); weeklyChart.invalidate();
        ...
        for (Map.Entry<String, Integer> entry : allData.entrySet()) {
            try {
                ...
                if (yr == selectedMonthlyYear) monthTotals.put(mo, monthTotals.getOrDefault(mo, 0) + entry.getValue());
            } catch (Exception ignored) {}
        }
        List<BarEntry> barEntries = new ArrayList<>(); List<String> monthlyLabels = new ArrayList<>();
        String[] months = {"Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"};
        for (int m = 0; m < 12; m++) { barEntries.add(new BarEntry(m, monthTotals.getOrDefault(m, 0))); monthlyLabels.add(months[m]); }
        BarDataSet barSet = new BarDataSet(barEntries, "Egg Count");
        barSet.setColor(Color.parseColor("#466d1d")); barSet.setDrawValues(true);
        monthlyChart.setData(new BarData(barSet)); monthlyChart.getXAxis().setValueFormatter(new IndexAxisValueFormatter(monthlyLabels));
        monthlyChart.getXAxis().setLabelRotationAngle(-45f); monthlyChart.getXAxis().setGranularity(1f); monthlyChart.getXAxis().setLabelCount(monthlyLabels.size());
        monthlyChart.animateY(800); monthlyChart.invalidate();
    ...
    private void calculatePerformance(int latest, double average) {
        ...
        if (performanceTitle != null) performanceTitle.setText(d >= 10 ? "Excellent Performance!" : d >= -5 ? "Stable Production" : "Production Alert");
        if (performanceDesc  != null) performanceDesc.setText(String.format(Locale.getDefault(), "You're %.1f%% %s average production this month", Math.abs(d), d >= 0 ? "above" : "below"));
    }

    private void generateSmartTip(int latest, int prev, double profit) {
        String tip;
        if (latest < prev * 0.8)      tip = "Urgent: Production dropped significantly (>20%). Check for diseases or extreme environmental stress immediately.";
        else if (latest < prev * 0.9) tip = "Critical Tip: Production dropped by 10%. Check for sudden temperature changes or stressors in the cages.";
        else if (latest > prev * 1.2) tip = "Outstanding! Yield is up by 20%. Analyze if recent changes in feed or lighting contributed to this boost.";
        else if (latest > prev * 1.1) tip = "Great Job! Yield is up. Ensure consistent feed supply to maintain this peak production rate.";
        else if (profit < 0)          tip = "Financial Alert: Inventory value is currently exceeding egg revenue. Review feed efficiency and waste management.";
        else if (latest > 0 && latest < 10) tip = "Startup Tip: Low volume detected. Ensure your quails have reached laying age (usually 6-7 weeks).";
        else {
            String[] t = {"Maintenance Tip: Production is stable. Remember to maintain 14-16 hours of daily light exposure for quails.",
            ...
            tip = t[new Random().nextInt(t.length)];
        }
    ...
    private void showReportDialog() {
        ...
        String summary =
                "📋  FARM ANALYTICS REPORT\n" +
                        ...
                        "📈  NET PROFIT\n" +
                        "   Net Profit           : " + currencyFormat.format(profit) + (profit < 0 ? "  ⚠ Loss" : "  ✔ Profit") + "\n\n" +
                        "🏆  PERFORMANCE\n" +
                        "   " + perfTitle + "\n" +
                        "   " + perfDesc + "\n\n" +
                        "💡  AI RECOMMENDATION\n" +
    ...
    private void generatePdfReport() {
        try {
            ...
            drawRow(canvas, "Net Profit",             currencyFormat.format(profit),             y, profit < 0 ? Color.parseColor("#C62828") : Color.parseColor("#1565C0")); y += 14;
            ...
            y = drawWrappedText(canvas, tip, 24, y, 550, body); y += 6;
            canvas.drawLine(20, y, 575, y, divider);
            ...
            String filename = "FarmReport_" + new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(new Date()) + ".pdf";
    ...
    private void drawSectionTitle(Canvas canvas, String title, int y) {
        Paint p = new Paint(); p.setTextSize(15f); p.setFakeBoldText(true); p.setColor(Color.parseColor("#466d1d"));
    ...
    private int drawWrappedText(Canvas canvas, String text, int x, int y, int maxWidth, Paint paint) {
        ...
        if (line.length() > 0) { canvas.drawText(line.toString(), x, y, paint); y += (int)(paint.getTextSize() * 1.4f); }
        return y;
    }
}
