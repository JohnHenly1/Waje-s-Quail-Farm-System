...
public class AnalyticsActivity extends AppCompatActivity {
    ...
    // Who's currently logged in, used to stamp "Generated by" on exported reports.
    // Pulled from the launching Intent's "username" / "role" extras (same "username"
    // extra already used for back-navigation below); falls back to generic labels if
    // either extra wasn't supplied so report generation never crashes on a null.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Resolve the logged-in user's name/role for the "Generated by" line stamped on
        // exported reports. Same "username" extra other activities pass around; "role"
        // is optional and falls back to a generic label if the launching screen didn't set it.
        currentUsername = getIntent().getStringExtra("displayName");
    ...
    /**
     * Maps a stored role code (e.g. "owner", "staff") to the display string that should
     * appear on exported reports. Falls back to the raw value (capitalized) for any role
     * that isn't one of the known codes, so an unexpected value never disappears silently.
     */
    private String getRoleDisplayName(String roleCode) {
        if (roleCode == null || roleCode.trim().isEmpty()) {
            return "Farm Staff";
        }
        switch (roleCode.trim().toLowerCase(Locale.getDefault())) {
            case "owner":
                return "Farm Owner";
            case "staff":
                return "Farm Staff";
            default:
                // Unknown role code — show it as-is rather than mislabeling the user.
    ...
    /**
     * "Generated by" line stamped on every exported report (dialog, PDF, PNG/JPEG).
     * Username and role are kept as clearly separate fields (rather than the role
     * jammed into parentheses after the name) and the role is shown using its
     * display string (e.g. "owner" -> "Farm Owner", "staff" -> "Farm Staff").
     */
    private String getGeneratedByLabel() {
        return "Generated by: " + currentUsername + "   |   Role: " + getRoleDisplayName(currentUserRole);
    }
    ...
    private void showReportDialog() {
        ...
        View dialogView = getLayoutInflater().inflate(R.layout.dialog_farm_report, null);

        TextView titleView = dialogView.findViewById(R.id.reportDialogTitle);
        TextView periodView = dialogView.findViewById(R.id.reportDialogPeriod);
        TextView generatedView = dialogView.findViewById(R.id.reportDialogGenerated);
        TextView generatedByView = dialogView.findViewById(R.id.reportDialogGeneratedBy);
        TextView totalView = dialogView.findViewById(R.id.reportDialogTotal);
        TextView gradeAView = dialogView.findViewById(R.id.reportDialogGradeA);
        TextView gradeBView = dialogView.findViewById(R.id.reportDialogGradeB);
        TextView gradeCView = dialogView.findViewById(R.id.reportDialogGradeC);

        titleView.setText(getString(R.string.farm_analytics_report_title));
        periodView.setText(periodLabel);
        generatedView.setText(getString(R.string.generated, ts));
        generatedByView.setText(getGeneratedByLabel());
        totalView.setText(String.valueOf(total));
        gradeAView.setText(String.valueOf(a));
        gradeBView.setText(String.valueOf(b));
        gradeCView.setText(String.valueOf(c));

        AlertDialog dialog = new AlertDialog.Builder(this)
                .setView(dialogView)
                .create();

        // Let our own rounded white card show through instead of the system dialog square
        if (dialog.getWindow() != null) {
            dialog.getWindow().setBackgroundDrawable(
                    new android.graphics.drawable.ColorDrawable(Color.TRANSPARENT));
        }

        int finalA = a, finalTotal = total, finalB = b, finalC = c;
        dialogView.findViewById(R.id.btnExportPdf).setOnClickListener(v -> {
            dialog.dismiss();
            generatePdfReport(finalTotal, finalA, finalB, finalC, periodLabel);
        });
        dialogView.findViewById(R.id.btnExportImage).setOnClickListener(v -> {
            dialog.dismiss();
            showImageFormatChooser(finalTotal, finalA, finalB, finalC, periodLabel);
        });
        dialogView.findViewById(R.id.btnCloseReport).setOnClickListener(v -> dialog.dismiss());
    ...
    private void showImageFormatChooser(int total, int a, int b, int c, String periodLabel) {
        final int GREEN = Color.parseColor("#355E1A");
        String[] formats = { getString(R.string.png_best_quality), getString(R.string.jpeg_smaller_file) };

        // Custom title: green bold text on white, with a thin green accent line underneath
        LinearLayout titleContainer = new LinearLayout(this);
        titleContainer.setOrientation(LinearLayout.VERTICAL);
        titleContainer.setBackgroundColor(Color.WHITE);

        TextView titleView = new TextView(this);
        titleView.setText(getString(R.string.choose_image_format));
        titleView.setTextColor(GREEN);
        titleView.setTextSize(20f);
        titleView.setTypeface(titleView.getTypeface(), android.graphics.Typeface.BOLD);
        titleView.setPadding(dpToPx(20), dpToPx(20), dpToPx(20), dpToPx(12));
        titleContainer.addView(titleView);

        View accentLine = new View(this);
        accentLine.setBackgroundColor(GREEN);
        LinearLayout.LayoutParams lineParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, dpToPx(2));
        lineParams.setMargins(dpToPx(20), 0, dpToPx(20), 0);
        titleContainer.addView(accentLine, lineParams);

        // White rows with black text (prevents dark-theme from turning them white-on-white)
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(
                this, android.R.layout.simple_list_item_1, formats) {
            @Override
            public View getView(int position, View convertView, android.view.ViewGroup parent) {
                ...
                if (row instanceof TextView) {
                    ...
                    tv.setTextColor(Color.BLACK);
                    tv.setTextSize(16f);
                    tv.setPadding(dpToPx(20), dpToPx(16), dpToPx(20), dpToPx(16));
                }
        ...
        dialog.setOnShowListener(d -> {
            ...
            // Subtle light-gray dividers between options
            if (dialog.getListView() != null) {
                dialog.getListView().setDivider(
                        new android.graphics.drawable.ColorDrawable(Color.parseColor("#E5E7EB")));
                dialog.getListView().setDividerHeight(1);
            }
            // Green Cancel button
            Button cancelBtn = dialog.getButton(AlertDialog.BUTTON_NEGATIVE);
            if (cancelBtn != null) {
                cancelBtn.setTextColor(GREEN);
            }
        });
    ...
    private void generatePdfReport(int total, int a, int b, int c, String periodLabel) {
        try {
            ...
            // Height bumped from 842 (plain A4) to 900 to make room for the extra
            // "Generated by" line in the header band without clipping the pie chart.
            PdfDocument.PageInfo pageInfo = new PdfDocument.PageInfo.Builder(595, 900, 1).create();
            PdfDocument.Page page = document.startPage(pageInfo);
            Canvas canvas = page.getCanvas();

            Paint bg = new Paint(); bg.setColor(Color.WHITE);
            canvas.drawRect(0, 0, 595, 900, bg);

            // Header band height increased from 92 -> 110 to fit the added
            // "Generated by" line alongside the existing generated-timestamp/period lines.
            Paint hdr = new Paint(); hdr.setColor(Color.parseColor("#355E1A"));
            canvas.drawRect(0, 0, 595, 110, hdr);
            ...
            canvas.drawText(getGeneratedByLabel(), 78, 94, hs);
    ...
    private void exportImageReport(int total, int a, int b, int c, String periodLabel, Bitmap.CompressFormat format, String ext, String mimeType) {
        try {
            // Height increased (870 -> 890) to fit the extra "Generated by" header line
            // without clipping the pie chart below it.
            int width = 595, height = 890;
            ...
            // Header band height increased from 92 -> 110, matching the PDF export, to fit
            // the added "Generated by" line.
            Paint hdr = new Paint(); hdr.setColor(Color.parseColor("#355E1A"));
            canvas.drawRect(0, 0, width, 110, hdr);
